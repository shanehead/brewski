...
Replace all monochrome outline SVG icons with custom multi-color filled "app-style" icons — chunky, bold, and expressive like iOS/macOS system icons. Icons are bare (no badge container); the icon shape itself carries all the color and personality.

## Scope

Two surfaces:

**Nav rail** (`AppShell.svelte`): Recipes, Batches, Tools, Equipment, Library, Settings  
**Tab bar** (via `BrewingIcon.svelte` / `icons.ts`): overview, ingredients, mash, water, fermentation, notes, batches  
**Ingredient type icons** (also via `icons.ts`): fermentable, hop, yeast

## Design Principles

- Icons use hard-coded fill colors, not `stroke="currentColor"`. They are always colorful regardless of context.
- **Inactive**: icon rendered at `opacity: 0.45`
- **Active (nav rail)**: icon at full opacity + existing `box-shadow` glow (already implemented)
- **Active (tab bar)**: icon at full opacity; underline indicator remains as-is
- All icons use a 24×24 viewBox, rendered at 22px on nav rail and 20px on tab bar
- Multiple fill layers per icon create depth: base shape, darker half for shadow, lighter accent details

## Color Assignments

| Icon | Primary | Shadow | Accent |
|------|---------|--------|--------|
| Recipes | `#3b82f6` (blue) | `#2563eb` | `#93c5fd` |
| Batches / batches tab | `#10b981` (emerald) | `#059669` | `#6ee7b7` |
| Tools | `#f59e0b` (amber) | `#d97706` | `#fcd34d` |
| Equipment | `#8b5cf6` (violet) | `#7c3aed` | `#c4b5fd` |
| Library | three-color stack: `#ec4899` / `#f97316` / `#06b6d4` | spine darks | — |
| Settings | `#64748b` (slate) | `#475569` | `#94a3b8` |
| Overview | `#3b82f6` | `#2563eb` | `#93c5fd` |
| Ingredients | `#84cc16` (lime, hop) + `#f59e0b` (amber, grain) | — | — |
| Mash | `#f97316` (orange) | `#ea580c` | `#fed7aa` |
| Water | `#38bdf8` (sky) | `#0284c7` | white highlight |
| Fermentation | `#8b5cf6` (violet) | `#7c3aed` | `#c4b5fd` bubbles |
| Notes | `#fbbf24` (yellow) | `#f59e0b` | white lines |
| Fermentable | `#f59e0b` (amber) | `#d97706` | `#fef3c7` |
| Hop | `#22c55e` (green) | `#16a34a` | `#bbf7d0` |
| Yeast | `#14b8a6` (teal) | `#0d9488` | white bubbles |

## SVG Artwork

All paths go in `src/lib/icons.ts`. The `ICONS` record maps `BrewingIconName` → inner SVG markup (no `<svg>` wrapper). Nav-specific icon names are added to the union type.

### New icon names added to `BrewingIconName`

```
"recipes" | "tools" | "equipment" | "library" | "settings"
```

(Existing names kept: `"fermentable" | "hop" | "yeast" | "overview" | "ingredients" | "mash" | "water" | "fermentation" | "notes" | "batches"`)

### Icon SVG paths

**recipes** — Open book, two pages, bookmark ribbon
```svg
...
**batches** — Three stacked barrels/kegs
```svg
...
**equipment** — Brew kettle with handles and lid
```svg
...
**library** — Three books of different heights and colors
```svg
...
**settings** — Gear with inner circle cutout
```svg
...
**ingredients** — Hop cone (left) + grain stalk (right)
```svg
...
**water** — Water drop with highlight
```svg
...
**fermentation** — Fermenter vessel with airlock and bubbles
```svg
...
**notes** — Notepad with lines and pen mark
```svg
...
**yeast** — Erlenmeyer flask with bubbles
```svg
...
- Add new names to `BrewingIconName` union: `"recipes" | "tools" | "equipment" | "library" | "settings"`
- Replace all existing SVG path strings in `ICONS` with the new filled artwork above

### `src/lib/components/BrewingIcon.svelte`

- Change `stroke="currentColor"` → `fill="none"` (no default stroke; fills are hard-coded in paths)
- Remove `stroke-width`, `stroke-linecap`, `stroke-linejoin` attributes from the `<svg>` wrapper
- Keep `width`, `height`, `viewBox`, `aria-hidden`, `data-icon`

### `src/lib/desktop/AppShell.svelte`

- Import `BrewingIcon` component
- Replace the 6 inline `<svg>` elements with `<BrewingIcon name="recipes"/>`, `<BrewingIcon name="batches"/>`, etc.
- Keep existing active/inactive style logic but change:
  - Active: `opacity: 1` (full color, existing glow)
  - Inactive: `opacity: 0.45`
  - Remove `color: #fff` from active (icons no longer use `currentColor`)
  - Remove `color: var(--color-text-secondary)` from inactive (icons no longer use `currentColor`)

### `src/lib/components/TabBar.svelte`

- Tab icon active state: full opacity via the existing active class/style
- Tab icon inactive state: `opacity: 0.45` applied to the `<BrewingIcon>` wrapper

### Mobile (`src/lib/mobile/BottomTabBar.svelte`)

- Same opacity treatment as desktop TabBar

## What Does NOT Change

- CSS custom properties — all theming untouched
- The `BrewingIcon` component API (`name` prop) — callers unchanged except AppShell
- Icon sizes (18px nav rail, 18px tab bar currently — bumped to 22px nav / 20px tab for better visibility at filled style)
- The tab bar underline active indicator
