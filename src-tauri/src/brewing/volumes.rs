...
mod tests {
    ...
    fn test_boil_volumes_standard() {
        ...
        assert!(
            (pre - 27.8).abs() < 0.5,
            "pre_boil was {pre:.2}L, expected ~27.8L"
        );
    }

    #[test]
    fn test_mash_tun_loss_increases_pre_boil() {
        let (pre_no_loss, _, _) = calculate_boil_volumes(23.0, 60.0, 10.0, 1.0, 1.0, 0.0, 0.0, 0.0);
        let (pre_with_loss, _, _) =
            calculate_boil_volumes(23.0, 60.0, 10.0, 1.0, 1.0, 0.0, 2.0, 0.0);
        assert!(
    ...
    fn test_hlt_deadspace_returned_in_total_water() {
        let (pre, _post, total) = calculate_boil_volumes(23.0, 60.0, 10.0, 1.0, 1.0, 0.0, 0.0, 2.0);
        assert!(
