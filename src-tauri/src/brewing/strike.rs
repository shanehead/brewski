...
mod tests {
    ...
    fn test_strike_temp_reference_value() {
        ...
        assert!(
            (result - 73.42).abs() < 0.1,
            "expected ~73.42°C, got {result:.2}"
        );
    }
    ...
    fn test_strike_temp_higher_ratio_needs_less_heating() {
        ...
        assert!(
            ...
            "lower ratio should require higher strike temp"
        );
    }
    ...
    fn test_strike_temp_cold_grain_needs_hotter_strike() {
        ...
        assert!(
            cold_grain > warm_grain,
            "colder grain should require higher strike temp"
        );
    }
}
